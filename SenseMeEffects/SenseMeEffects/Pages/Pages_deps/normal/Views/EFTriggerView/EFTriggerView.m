...
//        [self addSubview:self.txtLabel];
        ...
        dispatch_queue_t queue = dispatch_queue_create("com.SoftSugar.sticker.timer", NULL);
...
- (void)showTriggerViewWithContents:(NSArray <NSString *> *)contents images:(NSArray <UIImage *> *)images {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (NSInteger i = 0; i < contents.count; i ++) {
            ...
            NSLog(@"%@ - %@", NSStringFromCGRect(currentImageView.frame), NSStringFromCGRect(currentLabel.frame));
        }
