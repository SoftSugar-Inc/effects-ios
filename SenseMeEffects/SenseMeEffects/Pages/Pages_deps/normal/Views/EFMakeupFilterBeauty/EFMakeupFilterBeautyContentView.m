...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    id<EFDataSourcing> selectedModel = self.dataSource[indexPath.row];
    if ([selectedModel.efName isEqualToString:@"GAN肤质"]) {
        if ([EFMachineVersion isBeforeiPhone12]) {
            MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.numberOfLines = 0;
            hud.label.text = NSLocalizedString(@"您的设备型号暂不支持本功能，为了获得更好的体验，建议您使用iphone12及以上的设备。", nil);
            [hud hideAnimated:YES afterDelay:3];
            return;
        } else if (([UIDevice currentDevice].systemVersion.doubleValue < 16.0)) {
            MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self animated:YES];
            hud.mode = MBProgressHUDModeText;
            hud.label.numberOfLines = 0;
            hud.label.text = NSLocalizedString(@"您的系统版本过低，无法支持本功能，为了获得最佳体验，建议您更新系统至ios16及以上。", nil);
            [hud hideAnimated:YES afterDelay:3];
            return;
        }
    }
