...
- (void)setUI {
    ...
    NSArray *imageList = @[@"texiao_process", @"", @"lvjing_process", @"meiyan_process"];
    NSArray *titleList = @[@"特效", @"", @"滤镜", @"美颜"];
...
- (void)effectsWithDark:(BOOL)dark {
    ...
    NSArray *imageList = @[@"texiao", @"", @"lvjing", @"meiyan"]; //_process _dark
    for (UIButton *button in self.buttonList) {
        NSString *reImageName = imageList[button.tag];
        if (reImageName && reImageName.length > 0) {
            NSString *str = dark ? @"dark" : @"process";
            NSString *imageName = [NSString stringWithFormat:@"%@_%@", reImageName, str];
