...
typedef NS_ENUM(NSInteger, EFMovieRecorderStatus) {
	EFMovieRecorderStatusIdle = 0,
	EFMovieRecorderStatusPreparingToRecord,
	EFMovieRecorderStatusRecording,
	EFMovieRecorderStatusFinishingRecordingPart1,
	EFMovieRecorderStatusFinishingRecordingPart2,
	EFMovieRecorderStatusFinished,
	EFMovieRecorderStatusFailed
};


@interface EFMovieRecorder ()
{
    AVAssetWriter *_assetWriter;
	EFMovieRecorderStatus _status;
	NSURL *_URL;
	...
	__weak id<EFMovieRecorderDelegate> _delegate;
...
@implementation EFMovieRecorder
...
- (void)addVideoTrackWithSourceFormatDescription:(CMFormatDescriptionRef)formatDescription transform:(CGAffineTransform)transform settings:(NSDictionary *)videoSettings {
	...
	@synchronized(self) {
		if (_status != EFMovieRecorderStatusIdle) {
...
- (void)prepareToRecord {
	@synchronized(self) {
		...
		[self transitionToStatus:EFMovieRecorderStatusPreparingToRecord error:nil];
	}
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        @autoreleasepool {
            ...
            @synchronized(self) {
                if (error) {
                    [self transitionToStatus:EFMovieRecorderStatusFailed error:error];
                }
                else {
                    [self transitionToStatus:EFMovieRecorderStatusRecording error:nil];
                }
            }
        }
    });
...
- (void)finishRecording {
	@synchronized(self) {
		...
		switch (_status) {
			case EFMovieRecorderStatusIdle:
			case EFMovieRecorderStatusPreparingToRecord:
			case EFMovieRecorderStatusFinishingRecordingPart1:
			case EFMovieRecorderStatusFinishingRecordingPart2:
			case EFMovieRecorderStatusFinished:
//                @throw [NSException exceptionWithName:NSInternalInconsistencyException reason:@"Not recording" userInfo:nil];
                break;
			case EFMovieRecorderStatusFailed:
				...
				break;
			case EFMovieRecorderStatusRecording:
				shouldFinishRecording = YES;
				break;
		}
		
		if (shouldFinishRecording) {
			[self transitionToStatus:EFMovieRecorderStatusFinishingRecordingPart1 error:nil];
		}
		else {
			return;
		}
	...
	dispatch_async(_writingQueue, ^{
		...
		@autoreleasepool {
			@synchronized(self) {
				...
				if (_status != EFMovieRecorderStatusFinishingRecordingPart1) {
					return;
				}
				
				// It is not safe to call -[AVAssetWriter finishWriting*] concurrently with -[AVAssetWriterInput appendSampleBuffer:]
				// We transition to EFMovieRecorderStatusFinishingRecordingPart2 while on _writingQueue, which guarantees that no more buffers will be appended.
				[self transitionToStatus:EFMovieRecorderStatusFinishingRecordingPart2 error:nil];
			}
			...
			[_assetWriter finishWritingWithCompletionHandler:^{
				@synchronized(self) {
					...
					if (error) {
						...
						[self transitionToStatus:EFMovieRecorderStatusFinished error:nil];
					}
				}
			}];
		}
...
- (void)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer ofMediaType:(NSString *)mediaType {
	...
	@synchronized(self) {
		if (_status < EFMovieRecorderStatusRecording) {
...
- (void)transitionToStatus:(EFMovieRecorderStatus)newStatus error:(NSError *)error {
	BOOL shouldNotifyDelegate = NO;
	
#if DEBUG
	NSLog(@"EFMovieRecorder state transition: %@->%@", [self stringForStatus:_status], [self stringForStatus:newStatus]);
#endif
	
	if (newStatus != _status) {
		// terminal states
		if ((newStatus == EFMovieRecorderStatusFinished) || (newStatus == EFMovieRecorderStatusFailed)) {
			shouldNotifyDelegate = YES;
			// make sure there are no more sample buffers in flight before we tear down the asset writer and inputs
			
			dispatch_async(_writingQueue, ^{
				[self teardownAssetWriterAndInputs];
				if (newStatus == EFMovieRecorderStatusFailed) {
					[[NSFileManager defaultManager] removeItemAtURL:_URL error:NULL];
				}
			});
...
#if DEBUG
			if (error) {
				NSLog(@"EFMovieRecorder error: %@, code: %i", error, (int)error.code);
			}
#endif
		}
		else if (newStatus == EFMovieRecorderStatusRecording) {
			shouldNotifyDelegate = YES;
		}
...
- (NSString *)stringForStatus:(EFMovieRecorderStatus)status {
