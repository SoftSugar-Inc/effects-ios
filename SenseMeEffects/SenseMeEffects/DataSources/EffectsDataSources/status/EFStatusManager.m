...
static EFStatusManagerCodingKey const EFStatusManagerCodingStatusStorageKey = @"com.SoftSugar.damen_EFStatusManagerCodingStatusStorageKey";
static EFStatusManagerCodingKey const EFStatusManagerCodingStatusModelsKey = @"com.SoftSugar.damen_EFStatusManagerCodingStatusModelsKey";
    ...
    switch ([self efDownloadStatus:model]) {
        case EFMaterialDownloadStatusNotDownload: {
            ...
            [self efStartDownload:model
                ...
                if ([material.strID isEqualToString:currentID]) {
                    [self efModelSelected:material needMutex:needMutex];
                    if (completeSuccess) completeSuccess(material);
                }
        ...
        default: { // 已经下载 / 无需下载
            shouldSelectedModel = model;
            [self efModelSelected:model needMutex:needMutex];
            if (completeSuccess) completeSuccess(model);
            break;
        }
    }
}
...
//    [self.efStorageHelper efRemoveAllStickerFromCurrentStorageOnly:&currentStorage];
    [self.efCurrentStatusModels.efCachedStatusModels removeAllObjects];
    self.efStorageHelper = nil;
    NSArray * efCachedStatusModels = [[self.efCurrentStatusModels.efCachedStatusModels copy] sortedArrayUsingComparator:^NSComparisonResult(EFRenderModel *obj1, EFRenderModel *obj2) {
        return obj1.changetTime > obj2.changetTime;
    }];
    for (EFRenderModel * renderModel in efCachedStatusModels) {
        //        NSLog(@"恢复：%@ - %f", renderModel.efName, renderModel.efStrength);
...
/// 当前状态缓存到本地
-(BOOL)_efCache {
//    for (int i = 0; i < self.efCurrentStatusModels.efCachedStatusModels.count; i++) {
//        EFRenderModel *modelx = self.efCurrentStatusModels.efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efCache] current style %@ id: %d", modelx.efName, modelx.efId);
//        }
//    }
//    
//    for (int i = 0; i < self.efStatusModels.efCachedStatusModels.count; i++) {
//        EFRenderModel *modelx = self.efStatusModels.efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efCache] cache style %@ id: %d", modelx.efName, modelx.efId);
//        }
//    }
...
#pragma mark - helper
+(NSArray *)allParamsWithMode:(NSNumber *)mode {
    EFStatusManager *manager = [EFStatusManager sharedInstanceWith:mode.integerValue];
    NSArray *efCachedStatusModels = [manager.efCurrentStatusModels.efCachedStatusModels copy];
    NSMutableArray *result = [NSMutableArray array];
    for (EFRenderModel * renderModel in efCachedStatusModels) {
        [result addObject:[NSString stringWithFormat:@"%@: %f", renderModel.efName, renderModel.efStrength]];
    }
    return result.copy;
}

+(void)clearAllWithMode:(NSNumber *)mode {
    EFStatusManager *manager = [EFStatusManager sharedInstanceWith:mode.integerValue];
    NSArray *efCachedStatusModels = [manager.efCurrentStatusModels.efCachedStatusModels copy];
    for (EFRenderModel * renderModel in efCachedStatusModels) {
        if (renderModel.efAction != EFRenderModelActionDeselect) {
            EFDataSourceModel *model = [[EFDataSourceModel alloc] init];
            model.efRoute = renderModel.efRoute;
            model.efType = renderModel.efType;
            [manager efClear:model];
        }
    }
}
