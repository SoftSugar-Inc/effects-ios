...
-(void)efReset:(id<EFDataSourcing>)targetModel withCurrentStorage:(EFStatusModels *__autoreleasing *)currentStorage cacheStorage:(EFStatusModels *__autoreleasing *)cacheStorage andDefault:(EFStatusModels *)defaultModels {
    ...
    //    } else {
    EFRenderModel *lastRenderModel = nil;
    for (NSInteger i = 0; i < currentFiltersModel.count; i ++) {
        currentFiltersModel[i].efAction = defaultFiltersModel[i].efAction;
        currentFiltersModel[i].efStrength = defaultFiltersModel[i].efStrength;
        ...
        EFRenderModel *currentModel = cachedFiltersModel[i];
        if (!(lastRenderModel && (lastRenderModel.efType >> 5 == currentModel.efType >> 5) && lastRenderModel.efStrength > 0)) {
            [self _efCallbak:currentFiltersModel[i]];
            lastRenderModel = currentModel;
        }
    }
    //    }
}
...
-(void)_efSelecteMakeup:(id<EFDataSourcing>)targetModel withStrength:(CGFloat)strength withCurrentStorage:(EFStatusModels *__autoreleasing *)currentStorage andCacheStorage:(EFStatusModels *__autoreleasing *)cacheStorage {
    ...
    if (storageModel) { // 如果已经被选中了，反选逻辑
        ...
        if (strength > -CGFLOAT_MAX) { // 有强度 改变强度逻辑
            ...
            cacheStorageFilterModel.changetTime = CFAbsoluteTimeGetCurrent();
        } else { // 没有强度 反选逻辑
            // 将model从current中移除
            [(*currentStorage).efCachedStatusModels removeObject:storageModel];
            // 穿透到cache
            ...
            storageModel.efAction = EFRenderModelActionDeselect;
            storageModel.changetTime = CFAbsoluteTimeGetCurrent();
        }
        // 回调
        [self _efCallbak:storageModel];
...
//    for (int i = 0; i < (*currentStorage).efCachedStatusModels.count; i++) {
//        EFRenderModel *modelx = (*currentStorage).efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efSelecteStyle before] current style %@ id: %d address: %p-%p", modelx.efName, modelx.efId, *currentStorage, modelx);
//        }
//    }
//    
//    for (int i = 0; i < (*cacheStorage).efCachedStatusModels.count; i++) {
//        EFRenderModel *modelx = (*cacheStorage).efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efSelecteStyle before] cache style %@ id: %d address: %p-%p", modelx.efName, modelx.efId, *currentStorage, modelx);
//        }
//    }
    ...
    if (storageModel) { // 如果被选中
        ...
        if (sameTypeModels && sameTypeModels.count > 0) { // 有其他类型的滤镜被选中
            ...
            for (EFRenderModel * selectedModel in sameTypeModels) {
                // 回调取消状态
//                NSLog(@"@mahaomeng 反选 %@ id:%d address: %p", selectedModel.efName, selectedModel.efId, selectedModel);
                selectedModel.efAction = EFRenderModelActionDeselect;
                [self _efCallbak:selectedModel];
            }
        }
        // 将model加入current
        [self _efTransformFromTargetModel:targetModel toRenderModelP:&storageModel];
        storageModel.efStrength = [EFGlobalSingleton sharedInstance].isMale ? (50 << 8 | 50) : (80 << 8 | 80);
        storageModel.changetTime = CFAbsoluteTimeGetCurrent();
        [(*currentStorage).efCachedStatusModels addObject:storageModel];
        // 添加结果穿透到cache
        [(*cacheStorage).efCachedStatusModels addObject:[storageModel copy]];
...
//        EFRenderModel *modelx = (*currentStorage).efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efSelecteStyle after] current style %@ id: %d address: %p-%p", modelx.efName, modelx.efId, *currentStorage, modelx);
//        }
//    }
//    
//    for (int i = 0; i < (*cacheStorage).efCachedStatusModels.count; i++) {
//        EFRenderModel *modelx = (*cacheStorage).efCachedStatusModels[i];
//        if ([modelx.efName isEqualToString:@"美颜1"] || [modelx.efName isEqualToString:@"美颜2"]) {
//            NSLog(@"@mahaomeng [_efSelecteStyle after] cache style %@ id: %d address: %p-%p", modelx.efName, modelx.efId, *currentStorage, modelx);
//        }
//    }
    // 回调 渲染
    [self _efCallbak:storageModel];
...
-(void)_efJudgeHadStyleAndSetUnactiveWithCurrentStorage:(EFStatusModels *__autoreleasing *)currentStorage andCacheStorage:(EFStatusModels *__autoreleasing *)cacheStorage {
    ...
    if (currentStyleModel && currentStyleModel.efAction != EFRenderModelActionUnactive) {
        ...
        if (cacheStyleModel) {
            ...
            cacheStyleModel.changetTime = CFAbsoluteTimeGetCurrent();
        }
    }
}
...
-(NSArray <EFRenderModel *>*)_efFetchAllSameTypeModelsLike:(id<EFDataSourcing>)targetModel from:(EFStatusModels *)currentStorage {
    ...
    if (realTypeValue == EFFECT_BEAUTY_3D_MICRO_PLASTIC) { // 3d微整形
        ...
        if ([seniorTypes containsObject:@(realTypeValue)]) { // 高阶瘦脸互斥逻辑
            filterPredicate = [NSPredicate predicateWithFormat:@"SELF.efType >> 5 IN %@", seniorTypes];
        } else if (realTypeValue == 999999991 /*EFFECT_BEAUTY_BASE_SKIN_SMOOTH*/) { // 身体磨皮与磨皮1、GAN磨皮的互斥逻辑
            filterPredicate = [NSPredicate predicateWithFormat:@"SELF.efType >> 5 == %ud || (SELF.efStrength > %f && (SELF.efType >> 5 == %ud && SELF.mode IN %@))", realTypeValue, 0.0, EFFECT_BEAUTY_BASE_FACE_SMOOTH, @[@(EFFECT_SMOOTH_FULL_IMAGE), @(EFFECT_SMOOTH_FACE_GAN)]];
        } else if (realTypeValue == EFFECT_BEAUTY_BASE_FACE_SMOOTH && (targetModel.mode == EFFECT_SMOOTH_FULL_IMAGE || targetModel.mode == EFFECT_SMOOTH_FACE_GAN)) { // 磨皮1、GAN磨皮与身体磨皮的互斥逻辑
            filterPredicate = [NSPredicate predicateWithFormat:@"SELF.efType >> 5 == %ud || SELF.efType >> 5 == %ud", realTypeValue, 999999991/*EFFECT_BEAUTY_BASE_SKIN_SMOOTH*/];
        }
        else {
...
-(NSArray *)efSpecialTypes {
    if (!_efSpecialTypes) {
        _efSpecialTypes = @[@303, @304, @307, @309, @310, @311, @312, @(EFFECT_BEAUTY_PLASTIC_EYE_HEIGHT), @(EFFECT_BEAUTY_PLASTIC_HAIRLINE), @(EFFECT_BEAUTY_TONE_COLOR_TONE), @(EFFECT_BEAUTY_TONE_COLOR_TEMPERATURE),
//                            @(EFFECT_BEAUTY_PLASTIC_BROW_HEIGHT),
//                            @(EFFECT_BEAUTY_PLASTIC_BROW_DISTANCE),
//                            @(EFFECT_BEAUTY_PLASTIC_BROW_THICKNESS),
//                            @(EFFECT_BEAUTY_PLASTIC_MOUTH_WIDTH),
//                            @(EFFECT_BEAUTY_PLASTIC_FULLER_LIPS),
