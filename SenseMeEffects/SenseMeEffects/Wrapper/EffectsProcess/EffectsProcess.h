...
@protocol EFEffectsProcessCommonObjectDelegate <NSObject>

- (void)updateCommonObjectPosition:(st_rect_t)rect;

@end

@protocol EFEffectsProcessItsMeDelegate <NSObject>
...
@property (nonatomic, weak) id<EFEffectsProcessCommonObjectDelegate> commonObjectDelegate;
@property (nonatomic, weak) id<EFEffectsProcessItsMeDelegate> itsMeDelegate;
...
- (BOOL)getTextureWithPixelBuffer:(CVPixelBufferRef)pixelBuffer texture:(GLuint*)texture cvTexture:(CVOpenGLESTextureRef*)cvTexture withCache:(CVOpenGLESTextureCacheRef)cache;
