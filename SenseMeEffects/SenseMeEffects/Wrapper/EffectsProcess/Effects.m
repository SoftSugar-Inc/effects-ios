...
extern bool ENABLE_SMALL_INPUT;
...
@interface Effects ()<EffectsAudioPlayerManagerDelegate>
{
    EffectsType _type;
}
...
- (void)createHandleWithType:(EffectsType)type context:(EAGLContext *)glContext{
    ...
    switch (type) {
        ...
        case EffectsTypePhoto:
            ...
            if (ret != ST_OK) {
                NSLog(@"st_mobile_effect_set_module_state_change_callback error %d", ret);
            }
            break;
    }
    st_mobile_effect_set_param(handle, EFFECT_PARAM_MAX_MEMORY_BUDGET_MB, 1000.0);
    st_mobile_effect_set_param(handle, EFFECT_PARAM_DISABLE_TRIGER_OVERLAP, 1);
...
- (st_result_t)setEffectType:(st_effect_beauty_type_t)type mode:(int)mode {
    st_result_t iRet = ST_OK;
    iRet = st_mobile_effect_set_beauty_mode(self.handle, type, mode);
    if (iRet != ST_OK) {
        NSLog(@"st mobile beautify set beautiy type %d failed: %d", type, iRet);
    }
    return iRet;
...
-(st_result_t)addPackage:(NSString *)packagePath packageId:(int *)packageId {
    st_result_t iRet = st_mobile_effect_add_package(self.handle, packagePath.UTF8String, packageId);
    return iRet;
}

-(st_result_t)changePackage:(NSString *)packagePath packageId:(int *)packageId {
    st_result_t iRet = st_mobile_effect_change_package(self.handle, packagePath.UTF8String, packageId);
    return iRet;
}
...
-(st_result_t)getDetectConfig:(uint64_t *)p_detect_config {
    return st_mobile_effect_get_detect_config(self.handle, p_detect_config);
}

-(uint64_t)getHumanTriggerActions {
    uint64_t triggerActions;
    st_result_t result = st_mobile_effect_get_human_trigger_actions(self.handle, ST_MOBILE_DETECT_HAND_GESTURE, &triggerActions);
    return triggerActions;
}

-(st_result_t)getCustomEventConfig:(uint64_t *)p_custom_event_config {
    return st_mobile_effect_get_custom_event_config(self.handle, p_custom_event_config);
}
...
- (GLuint)processTexture:(GLuint)inputTexture
    ...
    if (handle) {
        ...
        int iWidth = ENABLE_SMALL_INPUT ? width/2 : width;
        int iHeight = ENABLE_SMALL_INPUT ? height/2 : height;
        int iStride = ENABLE_SMALL_INPUT ? stride/2 : stride;
        //2.set input buffer
        st_image_t inputImage = {.data = inputData, .pixel_format = inputFormat, .width = iWidth, .height = iHeight, .stride = iStride, .time_stamp= 0.0};
        st_effect_in_image_t effectImag = {.image = inputImage, .rotate = ST_CLOCKWISE_ROTATE_0, .b_mirror = false};
        ...
        st_image_t out_img;
        memset(&out_img, 0x0, sizeof(out_img));
        
        if (img_out)
        {
            out_img.width = iWidth;
            out_img.height = iHeight;
            out_img.pixel_format = fmt_out;
            out_img.data = img_out;
            
            if (fmt_out == ST_PIX_FMT_NV12 || fmt_out == ST_PIX_FMT_NV21 || fmt_out == ST_PIX_FMT_YUV420P || fmt_out == ST_PIX_FMT_GRAY8)
            {
                out_img.stride = iWidth;
            }
            else if (fmt_out == ST_PIX_FMT_BGR888 || fmt_out == ST_PIX_FMT_RGB888)
            {
                out_img.stride = iWidth * 3;
            }
            else if (fmt_out == ST_PIX_FMT_BGRA8888 || fmt_out == ST_PIX_FMT_RGBA8888)
            {
                out_img.stride = iWidth * 4;
            }
            else
            {
                NSLog(@"output format not supported: %d", fmt_out);
            }
            
            if (out_img.stride != 0)
            {
                output_param.p_image = &out_img;
            }
        }
        ...
        EFFECTSTIMELOG(key)
        st_result_t iRet = [self renderWithHandle:handle inParam:&input_param outParam:&output_param];
...
-(st_result_t)renderWithHandle:(st_handle_t)handle inParam:(st_effect_render_in_param_t*)input_param outParam:(st_effect_render_out_param_t*)output_param {
    return st_mobile_effect_render(handle, input_param, output_param);
}
...
st_result_t _modul_state_change_callback(st_handle_t handle, const st_effect_module_info_t* p_module_info) {
    STMobileEffectModuleInfoWrapper *wrapper = [[STMobileEffectModuleInfoWrapper alloc] init];
    wrapper.moduleInfo = p_module_info;
    [[NSNotificationCenter defaultCenter] postNotificationName:@"st_wrapper_onApiCallback" object:wrapper];
    return ST_OK;
}
