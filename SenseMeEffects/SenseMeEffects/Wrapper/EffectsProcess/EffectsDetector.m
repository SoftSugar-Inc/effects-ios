...
-(st_result_t)addSubModel:(NSString *)modelPath {
    char *desc = [NSString stringWithFormat:@"@mahaomeng st_mobile_human_action_add_sub_model %@", [NSURL fileURLWithPath:modelPath].lastPathComponent].UTF8String;
    EFFECTSTIMELOG(key)
    st_result_t result = st_mobile_human_action_add_sub_model(_hDetector, modelPath.UTF8String);
    EFFECTSTIMEPRINT(key, desc);
    if ([modelPath containsString:@"Skin"]) { // 皮肤分割
        [EFGlobalSingleton sharedInstance].efHasSegmentCapability = result != ST_E_NO_CAPABILITY;
    }
    return result;
}
